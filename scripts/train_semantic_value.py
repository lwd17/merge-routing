...
from merge.search.local_value import q_sem  # noqa: E402
...
def load_examples(run_dir: Path):
    ...
    for line in (run_dir / "records.jsonl").read_text().splitlines():
        ...
        finals = {f["agent"]: f.get("f1", 0.0) for f in (r.get("finals") or [])}
        records[key] = (pool, slots, queries, routed,
                        r.get("question", ""), finals)

    # Per-agent answer-F1 baseline from the no-communication arm of the same
    # question. It supplies the third label condition of Appendix A.2 ("or
    # improves the receiver's answer F1"); without it the positive rate is
    # dominated by the rare new-gold event and the calibration term collapses
    # both heads to their bounds.
    no_comm_f1 = {}
    for (tid, arm), rec in records.items():
        if arm in ("no_comm", "gurc_none"):
            no_comm_f1[tid] = rec[5]
    ...
    for line in (run_dir / "transfer_log.jsonl").read_text().splitlines():
        ...
        if key not in records:
            continue
        pool, slots, queries, routed, question, finals = records[key]
        it = pool.get(row["mid"])
        if it is None:
            continue
        ...
        base = no_comm_f1.get(row["task_id"], {})
        improves_f1 = (i in base and i in finals
                       and finals[i] > base[i] + 1e-9)
...
def main():
    ...
    if np.mean([e["y"] for e in examples]) < 0.15:
        print("  WARNING: low positive rate -- the 0.2 BCE calibration term "
              "will push softplus(r) toward 0 and tanh(v) toward -1, which "
              "destroys the ABSOLUTE scale the allocation objective needs. "
              "Check that a no_comm arm is present in the run directory so "
              "the answer-F1 label condition can be evaluated.")
