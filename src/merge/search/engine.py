...
# Per-thread token accounting: Total Tokens counts input and output usage
# across ALL LLM calls (planning, control, answers, retries). A run collects
# it by calling reset_usage() then usage() on the same thread.
...
def usage() -> dict:
    return {
        ...
        "total_tokens": getattr(_usage, "prompt", 0) + getattr(_usage, "completion", 0),
...
def _record_usage(resp) -> None:
    ...
    if u is None:
        return
    _usage.prompt = getattr(_usage, "prompt", 0) + int(getattr(u, "prompt_tokens", 0) or 0)
    _usage.completion = (getattr(_usage, "completion", 0)
